...
def get_body(url: str, headers: dict) -> dict:
    """ 获取响应体内容。
...
def get_illust_ids1(uid: str, lang: str = "zh") -> list:
    ...
    body = get_body(url, local_headers)
    if not body:
        print(f"get illust ids error!!!")
        return []
...
def get_illust_url(illust_id: str, lang: str = "zh") -> tuple[list[str], int, str]:
    ...
    body = get_body(url, local_headers)
    if not body:
        print(f"get {illust_id} url error!!!")
        return [], 0, "unknown"
    ...
    if page_count == 1:
        ...
        body = get_body(url, local_headers)
        if not body:
            print(f"get {illust_id} urls error!!!")
            return [], 0, user_name
        try:
            for page in body:
